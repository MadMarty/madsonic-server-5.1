      ...
      ////////////////////
			...
			//////////////////// 
		      if (template.queryForInt("select count(*) from version where version = 144") == 0) {
			        ...
			        template.execute("insert into transcoding2(name, source_formats, target_format, step1) values('video->flv', 'avi mpg mpeg mp4 m4v mkv mov wmv ogv divx m2ts', 'flv', 'ffmpeg -ss %o -i %s -async 1 -b %bk -s %wx%h -ar 44100 -ac 2 -v 0 -f flv -c:v libx264 -preset superfast -threads 0 -')");		
			        template.execute("insert into transcoding2(name, source_formats, target_format, step1) values('video->mkv', 'avi mpg mpeg mp4 m4v mkv mov wmv ogv divx m2ts flv', 'mkv', 'ffmpeg -ss %o -i %s -c:v libx264 -preset superfast -b:v %bk -c:a libvorbis -f matroska -threads 0 -')");		
			        template.execute("insert into transcoding2(name, source_formats, target_format, step1) values('video->webm', 'avi mpg mpeg mp4 m4v mkv mov wmv ogv divx m2ts flv', 'webm', 'ffmpeg -ss %o -t %d -i %s -async 1 -vf lutyuv=y=val*1.3 -b %bk -s %wx%h -ar 44100 -ac 2 -v 0 -f webm -c:v libvpx -preset superfast -c:a libvorbis -threads 0 -')");		
